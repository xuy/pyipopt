...
add_int_option (PyObject * self, PyObject * args)
{
  ...
  if (ret)
    ...
    {
      return PyErr_Format (PyExc_ValueError, "%s is not a valid int option",
			   param);
    }
}
...
add_num_option (PyObject * self, PyObject * args)
{
  ...
  if (ret)
    ...
    {
      return PyErr_Format (PyExc_ValueError, "%s is not a valid num option",
			   param);
    }
}
...
initpyipopt (void)
{
  ...
  Py_InitModule3 ("pyipopt", ipoptMethods, "A hook between Ipopt and Python");
